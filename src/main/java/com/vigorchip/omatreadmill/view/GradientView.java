...
public class GradientView  extends View {
    ...
    private void init() {
        ...
        mPaint_Rec.setColor(Color.parseColor("#EE5C42"));
    ...
    private void getitem(float x, float y) {
        ...
        for (int a = 1; a<11; a++){
            ...
            Log.e("print",x+"_____"+leftx+"___"+rightx);
            if (x<leftx){
                continue;
            }
            if (leftx<=x&&x<=rightx){
                isClick=true;
                if (y>heigth/5*4||y<heigth/5+14*itemHeigth){//最大坡度不能超过12
                    break;
                }else {
    ...
    private void drawBar(Canvas canvas) {
        ...
        for (int a=0;a<10;a++) {
            if (data!=null) {
                ...
                re = new RectF(width / 3 + s, heigth / 5 * 4 , w, heigth / 5 * 4);
                canvas.drawText(0 + "", width*3/8 + s + itemWidth / 2-10, heigth / 5 * 4 -  itemHeigth - 5, mPaint_Text);
            }
            s = s+10+itemWidth;
            w =w+10+itemWidth;
